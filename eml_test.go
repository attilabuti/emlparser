...
`,
		...
		8: {
			...
			content:     fmt.Sprintf("GIF89a%s%s;", []byte{1}, []byte{1}),
		},
		9: {
	...
	for index, td := range testData {
		e, err := Parse(bytes.NewBufferString(td.mailData))
		if err != nil {
			t.Error(err)
		}
		...
		if td.htmlBody != e.BodyHtml {
			t.Errorf("[Test Case %v] Wrong html body. Expected: '%s', Got: '%s'", index, td.htmlBody, e.BodyHtml)
		}

		if td.textBody != e.BodyText {
			t.Errorf("[Test Case %v] Wrong text body. Expected: '%s', Got: '%s'", index, td.textBody, e.BodyText)
		}
		...
		if len(td.attachments) != len(e.Attachments) {
			...
			for _, ad := range td.attachments {
				...
				for i, ra := range attachs {
					...
					if ra.Filename == ad.filename && ra.MediaType == ad.mediaType {
						found = true
		...
		if len(td.embeddedFiles) != len(e.EmbeddedFiles) {
			...
			for _, ad := range td.embeddedFiles {
				...
				for i, ra := range embeds {
					...
					if ra.Cid == ad.cid && encoded == ad.base64data && ra.MediaType == ad.mediaType {
						found = true
